...
add_binds("normal", {
    ...
    buf("^\\w$",                    function (w, c) w:enter_cmd(":open wikipedia ") end),
    -- Privoxyfilterung der Seite anzeigen
    buf("^p$",                        function (w, c)
        w:enter_cmd(":tabopen http://config.privoxy.org/show-url-info?url=" .. (w.view.uri or "")) end),
    ...
    key({},          "d",           function (w, m) 
        -- close Window if we are on the last Tab
    ...
    buf("^Q$",                      function (w) w:close_win() end),
...
add_binds("insert", {
    ...
    key({"Control"},  "e",          function (w)
        ...
        if "false" ~= s then
            luakit.spawn_sync(string.format("mkdir -p %s", dir))
